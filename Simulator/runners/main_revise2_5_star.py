# -*- coding: utf-8 -*-
"""Run the controlled star-shaped nonconvex feasible-region experiment."""
import os
...
# ============ Configuration ============
EPS_INIT = 0.30    # Pronounced four-lobed non-convex domain for k = 4.
STAR_R = 1.0       # star base radius R
STAR_K = 4         # Number of petals (k=4 four-petaled flower; between petals k valley → ρ in k direction >1)
...
N_FACETS = 16      # Number of polygon sides (continued; the cusp may be under-covered, which is ρ revealed)
OUT = PROJECT_ROOT / 'results' / 'ds_proj_revise_V1' / 'synthetic_star'
PRETRAIN_CACHE = OUT / f'pretrain_warmstart_eps{EPS_INIT}_k{STAR_K}_nfacets{N_FACETS}.npz'
...
def star_boundary(R, eps, k, n=400):
    """star true Ω Boundary points (press θ orderly, direct plot closed) ."""
    th = np.linspace(0, 2 * np.pi, n)
    r = R * (1.0 + eps * np.cos(k * th))
    return np.column_stack([r * np.cos(th), r * np.sin(th)])


def get_polygon(model, eps):
    """take ε polygon at (A,b).FullNet The input is delta=ε−eps_init: during training _combine_batch Feeding is noise,
    frame handle initial_value+noise write to EC → What the model learns is delta→polygon(eps_init+delta).So query ε Must feed ε−eps_init."""
    delta = float(eps) - EPS_INIT
    with torch.no_grad():
...
def run_pretrain():
    case = case_star(eps_init=EPS_INIT, R=STAR_R, k=STAR_K, model_type='pretrainnet',
                     device=device, n_facets=N_FACETS)
...
def plot_evolution(name, snapshots, color, subtitle):
    """2×3 Grid: each subplot = star trueΩ(red) + Currently learned convex polygon(from x̂ Radial tracking {Ax≤b})."""
    x_ref = np.array([0.0, 0.0])
    dirs = gen_dirs_even(200)
    bnd_true = star_boundary(STAR_R, EPS_INIT, STAR_K, 400)   # true Ω border (press θ orderly)
    ...
    for idx, (it, A, b) in enumerate(snapshots):
        ax = axes[idx]
        ax.fill(np.append(bnd_true[:, 0], bnd_true[0, 0]),
                np.append(bnd_true[:, 1], bnd_true[0, 1]),
                color='#C44E52', alpha=0.13)
        ax.plot(np.append(bnd_true[:, 0], bnd_true[0, 0]),
                np.append(bnd_true[:, 1], bnd_true[0, 1]),
    ...
    fig.suptitle(f'{subtitle}  (ε={EPS_INIT}, k={STAR_K})', fontsize=11)
...
def train_version(name, radial_probe, A_pre, b_pre):
    ...
    Synchronous capture N_SNAP Zhang feasible region snapshot (constant ε=EPS_INIT assessment). Return (model, snapshots)."""
    case = case_star(eps_init=EPS_INIT, R=STAR_R, k=STAR_K, model_type='fullnet',
                     device=device, n_facets=N_FACETS)
    ...
    snapshots = []
    A0, b0 = get_polygon(model, EPS_INIT)                  # debut: warm-start (ZeroInit → model(0)=(A_pre,b_pre))
    ...
    def snap_callback(ecb, i_iter):
        ...
        if nxt < len(cb_targets) and cb_count[0] >= cb_targets[nxt]:
            A_s, b_s = get_polygon(model, EPS_INIT)
...
def calculate_rhos(models):
    ...
    k_om = np.array([star_k_omega(STAR_R, EPS_INIT, STAR_K, x_ref, v) for v in dirs])

    polys, rhos = {}, {}
    for name, m in models.items():
        A, b = get_polygon(m, EPS_INIT)
...
def plot_and_eval(models):
    ...
    bnd_true = star_boundary(STAR_R, EPS_INIT, STAR_K, 400)
    ...
    # true Ω (star fill) + true radial boundary
    axL.fill(np.append(bnd_true[:, 0], bnd_true[0, 0]),
             np.append(bnd_true[:, 1], bnd_true[0, 1]),
             color='#C44E52', alpha=0.15)
    axL.plot(np.append(bnd_true[:, 0], bnd_true[0, 0]),
             np.append(bnd_true[:, 1], bnd_true[0, 1]),
             '-', color='#C44E52', lw=2.2,
             label=r'true $\Omega$ (star, $r=R(1+\varepsilon\cos k\theta)$)')
    ...
    axL.set_xlim(-1.8, 1.8); axL.set_ylim(-1.8, 1.8)
    axL.set_aspect('equal'); axL.grid(linestyle='--', alpha=0.3)
    axL.set_xlabel(r'$x_1$'); axL.set_ylabel(r'$x_2$')
    axL.set_title(f'(a) Learned polygons vs true non-convex star '
                  f'(ε={EPS_INIT}, k={STAR_K})', fontsize=10)
    ...
    axR.set_title('(b) Reference-point coverage ρ (valleys → ρ>1 over-cover)', fontsize=10, pad=14)
    axR.legend(fontsize=8, loc='lower center', bbox_to_anchor=(0.5, -0.13))
    axR.grid(linestyle='--', alpha=0.3)

    fig.suptitle('Controlled non-convex star Ω: projection probing vs '
                 'reference-point radial probing', fontsize=12)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    out_png = OUT / 'star_radial_vs_projection.png'
    out_svg = OUT / 'star_radial_vs_projection.svg'
    fig.savefig(out_png, dpi=150)
    fig.savefig(out_svg)

    # ---- Summary ----
    print(f"\n=== ρ Summary (ε={EPS_INIT}, k={STAR_K}; Intervalve valley direction ρ>1=non-convex signature) ===")
    for name in models:
        rho = rhos[name]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='R2.5Analyzing star-shaped non-convex domains: Projective supervision versus radial supervision')
    parser.add_argument(
