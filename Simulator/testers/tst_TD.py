...
for dsppc_name in dscases:
    ...
    for tsppc_name in tscases:
        ...
        dscasedata_dict = TD_case.define_td_case_data(tsppc, dsppc, ds_percent=0.75, load_threshold=20)  # Note that this will changetsppcload
        ...
        # This piece is to add approximation:
        ...
        model_base = None
        #Approximate model
        model = TD_case.TDcase(tscasedata=tsppc, dscasedata_dict=dscasedata_apx_dict, is_apx=True)  # is_apxControls whether the approximation
        ...
        model = TD_case.TDcase(tscasedata=tsppc, dscasedata_dict=dscasedata_dict, is_apx=False)  # is_apxControls whether the approximation
        num_constraints = sum(1 for _ in model.component_data_objects(pyo.Constraint, active=True, descend_into=True))
        num_vars = sum(1 for _ in model.component_data_objects(pyo.Var, active=True, descend_into=True))
...
df.to_excel(output_path, index=False)  # index=FalseAvoid writing row index
